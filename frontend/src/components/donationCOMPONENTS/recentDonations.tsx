   ...
   <div className='flex gap-[20px]'>
      ...
      <div>
        ...
        <p className='text-[10px] font-[300] text-white/50'>
          {Math.floor((Date.now() - new Date(donation.createdAt).getTime()) / (1000 * 60 * 60))}h ago
        </p>
      </div>
    </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setData(response.data.sentDonations || [])     
        ...
        <div className='flex flex-col w-full gap-[24px]'>
          ...
          <div className='flex flex-col gap-[12px]'>
            {Array.from({ length: 2 }).map((_, index) => (
              <Skeleton key={index} className='w-full h-[133px] rounded-[16px]' />
            ))}
          </div>
          <Skeleton className='w-full h-[43px]' />
        </div>
      ) : data.length === 0 ? (
        <div className="text-white py-4  flex w-full justify-center">No donations yet</div>
      ) : (
